<template>
    <div class="container">
      ...
      <div class="row">
          <!-- <template v-for="(data, index) in  displayedItems" :key="index"> -->
            <template v-for="(data, index) in displayedItems" @click="filteredCountries(index) ">
              <div class="col-md-3">
  ...
  export default {
    ...
    computed: {
      ...
      filteredCountries() {
      if (this.items.length === 0) return []; // just making sure that countries is already populated
